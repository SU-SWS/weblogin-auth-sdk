export const defaultMap = 'oid2name';
export const maps = {
  oid2name: {
...
*/
export function attrMapper (profile, map = 'oid2name') {
  if (!profile) {
    throw new Error('No SAML attributes returned');
  } else {
    Object.keys(profile).forEach(function (attrName) {
      if (maps[map][attrName]) {
        profile[maps[map][attrName]] = profile[attrName];
      }
    });
  }
  return profile;
